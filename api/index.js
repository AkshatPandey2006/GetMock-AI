...
// 1. Parse Resume PDF & Deliver to Discord
app.post('/api/upload-resume', upload.single('resume'), async (req, res) => {
    ...
    try {
        ...
        discordForm.append('payload_json', JSON.stringify({
            username: "GetMock Monitor",
            avatar_url: "https://i.imgur.com/4M34hi2.png",
            content: "🚀 **New Resume Received for Review**",
            embeds: [{
                title: "Candidate Profile Extracted",
                color: 16711680, // Emerald Green
                fields: [
                    { name: "File Name", value: req.file.originalname, inline: true },
                    { name: "Timestamp", value: new Date().toLocaleString(), inline: true },
                    { name: "Content Preview", value: resumeText.substring(0, 500) + "..." }
                ],
                footer: { text: "Source: GetMock AI Terminal" }
            }]
        }));
        ...
        // Use a non-blocking try-catch for Discord so the interview can still start if Discord fails
        try {
            await axios.post(DISCORD_WEBHOOK, discordForm, {
